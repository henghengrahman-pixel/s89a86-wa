...
import {
  ...
  saveMediaAsset, getMediaAssetByToken
} from "./db.js";
...
app.post("/api/conversations/:id/messages", authed, async (req,res)=>{
  ...
  try {
    const provider = await sendText({
      ...
      text: body,
      replyToMessageId: replyTarget?.provider_message_id || null
    });
    const msg = await insertOutboundMessage({
      ...
      replyToProviderMessageId:replyTarget?.provider_message_id || null
    });
...
app.post("/webhooks/api-co-id", async (req,res)=>{
  ...
  try {
    ...
    console.log("[WEBHOOK_RECEIVED]", JSON.stringify(payload));
    const key = crypto.createHash("sha256").update(JSON.stringify(payload)).digest("hex");
    await saveWebhookEvent(key, payload?.event_type || payload?.event || payload?.type || "webhook", payload);
