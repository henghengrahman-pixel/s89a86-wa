...
import {
  ...
  listQuickReplies, createQuickReply, updateQuickReply, deleteQuickReply, getQuickReply
} from "./db.js";
...
function validateQuickReplyInput(body = {}) {
  const shortcut = normalizeShortcut(body.shortcut || body.tag || '');
  ...
  if (!shortcut) throw new Error('Shortcut hashtag wajib diisi.');
  if (!content) throw new Error('Isi balasan hashtag wajib diisi.');
  return { shortcut, title: title || shortcut, content, sortOrder, isActive };
}
