...
import {
  ...
  listQuickReplies, createQuickReply, updateQuickReply, deleteQuickReply, getQuickReply, findQuickReplyShortcutConflict
} from "./db.js";
...
function validateQuickReplyInput(body = {}) {
  const rawShortcuts = Array.isArray(body.shortcuts) ? body.shortcuts : [body.shortcut || body.tag || ''];
  const shortcuts = Array.from(new Set(rawShortcuts.map(normalizeShortcut).filter(Boolean))).slice(0, 20);
  const shortcut = shortcuts[0] || normalizeShortcut(body.shortcut || body.tag || '');
  const title = String(body.title || shortcut || '').trim().slice(0, 120);
  const content = String(body.content || body.message || '').trim();
  const sortOrder = Math.max(0, Math.min(9999, Number(body.sortOrder || body.sort_order || 0) || 0));
  ...
  if (!shortcut || !shortcuts.length) throw new Error('Minimal 1 shortcut hashtag wajib diisi.');
  if (!content) throw new Error('Isi balasan hashtag wajib diisi.');
  return { shortcut, shortcuts, title: title || shortcut, content, sortOrder, isActive };
}
...
app.post('/api/quick-replies', authed, async (req,res)=>{
  try {
    const payload = validateQuickReplyInput(req.body || {});
    const conflict = await findQuickReplyShortcutConflict(payload.shortcuts);
    if (conflict) return res.status(409).json({ok:false,error:`Shortcut #${payload.shortcuts.find(x => (conflict.shortcuts || [conflict.shortcut]).includes(x)) || conflict.shortcut} sudah dipakai.`});
    const item = await createQuickReply(payload);
    res.json({ ok:true, item });
  } catch (e) {
    const status = e.code === '23505' ? 409 : 400;
    res.status(status).json({ ok:false, error: e.code === '23505' ? 'Hashtag sudah ada. Gunakan nama lain.' : e.message });
  }
});

app.patch('/api/quick-replies/:id', authed, async (req,res)=>{
  try {
    const existing = await getQuickReply(req.params.id);
    if (!existing) return res.status(404).json({ ok:false, error:'Hashtag tidak ditemukan.' });
    const payload = validateQuickReplyInput({
      shortcut: req.body?.shortcut ?? existing.shortcut,
      shortcuts: req.body?.shortcuts ?? existing.shortcuts ?? [existing.shortcut],
      title: req.body?.title ?? existing.title,
      content: req.body?.content ?? existing.content,
      sortOrder: req.body?.sortOrder ?? req.body?.sort_order ?? existing.sort_order,
      isActive: req.body?.isActive ?? req.body?.is_active ?? existing.is_active
    });
    const conflict = await findQuickReplyShortcutConflict(payload.shortcuts, req.params.id);
    if (conflict) return res.status(409).json({ok:false,error:`Shortcut #${payload.shortcuts.find(x => (conflict.shortcuts || [conflict.shortcut]).includes(x)) || conflict.shortcut} sudah dipakai.`});
    const item = await updateQuickReply(req.params.id, payload);
    res.json({ ok:true, item });
  } catch (e) {
    const status = e.code === '23505' ? 409 : 400;
    res.status(status).json({ ok:false, error: e.code === '23505' ? 'Hashtag sudah ada. Gunakan nama lain.' : e.message });
...
app.delete('/api/quick-replies/:id', authed, async (req,res)=>{
  const ok = await deleteQuickReply(req.params.id);
  if (!ok) return res.status(404).json({ ok:false, error:'Hashtag tidak ditemukan.' });
  res.json({ ok:true });
});
