...
export async function listQuickReplies({ query = '', limit = 100, activeOnly = false } = {}) {
  ...
  if (q) {
    ...
    clauses.push(`(LOWER(shortcut) LIKE $${params.length} OR LOWER(title) LIKE $${params.length} OR LOWER(content) LIKE $${params.length})`);
  }
...
export async function createQuickReply({ shortcut, title, content, isActive = true, sortOrder = 0 }) {
  const { rows } = await pool.query(`
    INSERT INTO quick_replies(shortcut, title, content, is_active, sort_order, updated_at)
    VALUES ($1,$2,$3,$4,$5,NOW())
    RETURNING *
  `, [shortcut, title, content, Boolean(isActive), Number(sortOrder || 0)]);
  return rows[0] || null;
}

export async function updateQuickReply(id, { shortcut, title, content, isActive = true, sortOrder = 0 }) {
  const { rows } = await pool.query(`
    ...
    WHERE id=$1
    RETURNING *
  `, [id, shortcut, title, content, Boolean(isActive), Number(sortOrder || 0)]);
  return rows[0] || null;
}
