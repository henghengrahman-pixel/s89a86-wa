...
export async function insertInboundMessage({ conversationId, contactId, providerMessageId, type, body, mediaUrl, rawPayload, createdAt }) {
    ...
    try {
      const r = await client.query(`
        INSERT INTO messages
          (conversation_id, contact_id, provider_message_id, direction, message_type, body, media_url, status, raw_payload, created_at, updated_at)
        VALUES ($1,$2,$3,'in',$4,$5,$6,'received',$7,$8,$8)
        RETURNING *
      `, [conversationId, contactId, providerMessageId || null, type || "text", body || "", mediaUrl || null, rawPayload, ts]);
      inserted = r.rows[0];
    } catch (e) {
...
export async function insertOutboundMessage({ conversationId, contactId, providerMessageId, body, rawPayload, sentBy, createdAt, status = "sent", messageType = "text" }) {
  const ts = createdAt ? new Date(createdAt) : new Date();
  let rows;
  try {
    const r = await pool.query(`
      INSERT INTO messages
        (conversation_id, contact_id, provider_message_id, direction, message_type, body, status, raw_payload, sent_by, created_at, updated_at)
      VALUES ($1,$2,$3,'out',$4,$5,$6,$7,$8,$9,$9)
      RETURNING *
    `, [conversationId, contactId, providerMessageId || null, messageType || "text", body || "", status || "sent", rawPayload || null, sentBy || null, ts]);
    rows = r.rows;
  } catch (e) {
    if (e.code === "23505" && providerMessageId) {
      const r = await pool.query(`SELECT * FROM messages WHERE provider_message_id=$1`, [providerMessageId]);
