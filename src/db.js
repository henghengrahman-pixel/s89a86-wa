...
export async function updateMessageStatus(providerMessageId, status) {
  ...
  const rank = { failed: 0, received: 1, sent: 2, delivered: 3, read: 4 };
  const incomingRank = rank[normalized] ?? 1;
  ...
  const { rows } = await pool.query(`
    ...
    RETURNING *
  `, [providerMessageId, incomingRank, normalized]);
...
export async function saveWebhookEvent(eventKey, eventType, payload) {
  try {
    ...
    console.error("[WEBHOOK_EVENT_SAVE_FAILED]", e.message);
    return true;
  }
}
