...
function renderMessages(messages){
  ...
  for(const m of messages){
    ...
    const closeInTime=prev &&
      Math.abs(new Date(m.created_at)-new Date(prev.created_at))<=12000;
    if(sameBody && closeInTime) continue;
    visible.push(m);
  }

  currentMessages=visible;
  msgEl.innerHTML=visible.map((m,i)=>{
    const prev=visible[i-1];
    const senderSwitch=prev && prev.direction!==m.direction ? " sender-switch" : "";
    return `
    <div class="message-row ${m.direction}${senderSwitch}">
...
composer.addEventListener("submit",async e=>{
  ...
  try{
    if(selectedFile){
      ...
      await api(`/api/conversations/${current.id}/media`,{
        method:"POST",
        body:fd
      });
      clearAttachment();
