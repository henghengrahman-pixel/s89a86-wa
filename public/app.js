...
function scheduleConversationReload(delay=350){
...
async function openConversation(id,{keepScroll=false}={}){
  ...
  // Tampilkan panel segera saat row diklik. Versi lama menunggu request selesai,
  // sehingga area kanan terlihat kosong dan terasa seperti tidak merespons.
  ...
  // Batalkan request chat sebelumnya bila user cepat pindah percakapan.
  openController?.abort();
  const controller=new AbortController();
  openController=controller;

  try{
    const d=await api(`/api/conversations/${id}/messages?limit=120`,{signal:controller.signal});
    if(seq!==openSeq) return;

    current=d.conversation;
    currentMessages=Array.isArray(d.messages)?d.messages:[];
    ...
    // Optimistis: hilangkan badge lokal tanpa menunggu reload seluruh list.
    ...
    document.querySelector("#retryChat")?.addEventListener("click",()=>openConversation(id));
    console.error("[OPEN_CHAT_FAILED]",err);
  }finally{
...
function mediaMarkup(m){
  ...
  if(type==="image") return `<a href="${esc(m.media_url)}" target="_blank" rel="noopener"><img class="msg-media" loading="lazy" decoding="async" src="${esc(m.media_url)}" alt="Gambar"></a>`;
  if(type==="video") return `<video class="msg-media" controls preload="none" src="${esc(m.media_url)}"></video>`;
  if(type==="audio") return `<audio controls preload="none" src="${esc(m.media_url)}"></audio>`;
...
socket.on("message:update",m=>{
  if(current?.id===m.conversation_id){
    ...
    if(idx>=0){
      ...
      messageCache.set(current.id,currentMessages);
      const row=msgEl.querySelector(`[data-message-id="${CSS.escape(String(currentMessages[idx].id))}"]`);
      const statusEl=row?.querySelector(".status");
      if(statusEl && m.status) statusEl.textContent=m.status;
    }
  }
  scheduleConversationReload(450);
});

socket.on("conversation:refresh",()=>scheduleConversationReload(400));
socket.on("conversation:update",()=>scheduleConversationReload(400));
