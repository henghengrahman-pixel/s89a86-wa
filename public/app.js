...
const shortcutChips = $("#shortcutChips");
...
function updateHeader() {
  ...
  archiveBtn.textContent = current.is_archived ? "Keluarkan arsip" : "Arsip";
}
...
function filteredQuickReplies(query = "") {
  ...
  return items.filter(item => {
    const aliases = (Array.isArray(item.shortcuts) && item.shortcuts.length ? item.shortcuts : [item.shortcut]).map(x=>String(x||"").toLowerCase());
    const title = String(item.title || "").toLowerCase();
    const content = String(item.content || "").toLowerCase();
    return aliases.some(x=>x.includes(q)) || title.includes(textNeedle) || content.includes(textNeedle);
  });
}
function renderSnippetMenu(items = []) {
  ...
  snippetMenu.innerHTML = items.map((item, idx) => `
    <button class="snippet-item ${idx===snippetMenuIndex?"active":""}" type="button" data-snippet-id="${esc(item.id)}">
      <div class="snippet-item-top"><span class="snippet-hash">${esc((Array.isArray(item.shortcuts)&&item.shortcuts.length?item.shortcuts:[item.shortcut]).map(x=>`#${x}`).join("  "))}</span><span class="snippet-label">${esc(item.title || item.shortcut)}</span></div>
...
function renderQuickReplyLibrary(activeId = snippetId.value || "") {
  ...
  const items = quickReplies.filter(item => {
    ...
    return [...(Array.isArray(item.shortcuts)?item.shortcuts:[item.shortcut]), item.title, item.content].join(" ").toLowerCase().includes(query);
  });
  snippetLibraryList.innerHTML = items.length ? items.map(item => `
    <div class="snippet-library-card ${activeId===item.id?"active":""}" data-edit-snippet="${esc(item.id)}">
      <div class="snippet-library-card-top">
        <div>
          <h4>${esc((Array.isArray(item.shortcuts)&&item.shortcuts.length?item.shortcuts:[item.shortcut]).map(x=>`#${x}`).join("  "))}</h4>
          <small>${esc(item.title || item.shortcut)}</small>
...
function renderShortcutChips() {
  shortcutChips.innerHTML = draftShortcuts.map(tag => `<span class="shortcut-chip">#${esc(tag)}<button type="button" data-remove-shortcut="${esc(tag)}" aria-label="Hapus #${esc(tag)}">×</button></span>`).join("");
}
function addDraftShortcut(value) {
  const tag = normalizeShortcut(value);
  if (!tag) return false;
  if (!draftShortcuts.includes(tag)) draftShortcuts.push(tag);
  draftShortcuts = draftShortcuts.slice(0,20);
  snippetShortcut.value = "";
  renderShortcutChips();
  return true;
}
function commitShortcutInput() {
  const raw = String(snippetShortcut.value || "").trim();
  if (raw) addDraftShortcut(raw);
}
shortcutChips.addEventListener("click", e => {
  const btn = e.target.closest("[data-remove-shortcut]");
  if (!btn) return;
  draftShortcuts = draftShortcuts.filter(x => x !== btn.dataset.removeShortcut);
  renderShortcutChips();
  snippetShortcut.focus();
});
snippetShortcut.addEventListener("keydown", e => {
  if (e.key === "Enter" || e.key === ",") {
    e.preventDefault();
    addDraftShortcut(snippetShortcut.value);
    return;
  }
  if (e.key === "Backspace" && !snippetShortcut.value && draftShortcuts.length) {
    draftShortcuts.pop();
    renderShortcutChips();
  }
});
snippetShortcut.addEventListener("blur", commitShortcutInput);
...
function fillSnippetForm(item) {
  ...
  draftShortcuts = Array.from(new Set((Array.isArray(item.shortcuts) && item.shortcuts.length ? item.shortcuts : [item.shortcut]).map(normalizeShortcut).filter(Boolean)));
  snippetShortcut.value = "";
  renderShortcutChips();
...
snippetForm.addEventListener("submit", async e => {
  ...
  if (!payload.shortcuts.length) return setStatusMessage("Minimal 1 shortcut wajib diisi.", "error");
